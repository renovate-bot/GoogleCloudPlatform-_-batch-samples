...
function exit_error() {
  ...
  echo "Batch CentOS Image Build Failed:" "$@"
  exit 1
}
...
function install_nfs() {
  ...
  if [[ "$install_nfs_packages" = "true" ]]; then
    yum -y install nfs-utils || exit_error "nfs installation failed."
  fi
}
...
function install_gpu_driver() {
  ...
  if [[ "$install_gpu_packages" = "true" ]]; then
    ...
    yum install -y epel-release pciutils gcc make dkms acpid libglvnd-glx libglvnd-opengl libglvnd-devel pkgconfig
    yum install -y python3 || exit_error "python3 installation failed."
